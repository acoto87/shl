...
| `Sort`(_typeName_* list, int32_t (*compareFn)(const _itemType_ item1, const _itemType_ item2, void* userdata), void* userdata) | Sort the list using the comparing function `compareFn`. This function must receive two elements `item1` and `item2` from the list and must return a value `< 0` if `item1 < item2`, a value `> 0` if `item1 > item2` and a value `= 0` if `item1 == item2`. The `userdata` pointer is passed through to each `compareFn` invocation. | void |
